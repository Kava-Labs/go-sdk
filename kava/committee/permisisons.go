...
import (
	...
	paramstypes "github.com/kava-labs/cosmos-sdk/x/params/types"
	upgrade "github.com/kava-labs/cosmos-sdk/x/upgrade"

	bep3types "github.com/kava-labs/go-sdk/kava/bep3"
	cdptypes "github.com/kava-labs/go-sdk/kava/cdp"
	pricefeedtypes "github.com/kava-labs/go-sdk/kava/pricefeed"
)
...
func (ams AllowedMarkets) Allows(current, incoming pricefeedtypes.Markets) bool {
	...
	for _, incomingM := range incoming {
		...
		var currentM pricefeedtypes.Market
		for _, p := range current {
			if p.MarketID != incomingM.MarketID {
				continue
			}
